...
public class TSBHashTableDA<K,V> implements Map<K,V>, Cloneable,Serializable
{
    ...
    // conteo de operaciones de cambio de tamaño.
    ...
    public V put(K key, V value)
    {
        ...
        else
        {
            ...
            int pos = search_for_OPENorTOMBSTONE(this.table, this.h(key));
    ...
    protected void rehash()
    {
        ...
        for(int i=0; i<this.table.length; i++)
        {
            ...
            if(x.getState() == CLOSED)
            {
                ...
                int y = search_for_OPENorTOMBSTONE(temp, ik);
     ...
     */
    private int search_for_index(K key, int ik) // no deberia acumular sino directamente valor a ik
    {
        for(int j=0; ;j++)
        {
            ik = (int)Math.pow(j, 2);
            ...
            if(entry == null){return -1;}   //linea agregaada por error en test
     ...
     */
    private int search_for_OPENorTOMBSTONE(Object t[], int ik)
    {
        for(int j=0; ;j++)
        {
            ik = (int)Math.pow(j, 2);
    ...
    private class EntrySet extends AbstractSet<Map.Entry<K, V>>
    {
        ...
        public boolean contains(Object o)
        {
            ...
            if(!((Entry<K,V>)o instanceof Entry)) return false; //lo castee porque lo pedia nose
            ...
            Entry<K, V> e = (Entry<K, V>) TSBHashTableDA.this.table[square_index];
            while (e.getState() != 0)
            {
                if (e.getState() == 1)
                {
                ...
                e = (Entry<K, V>) TSBHashTableDA.this.table[square_index];
            }
            return false;
        }
        ...
        public boolean remove(Object o)
        {
            ...
            if (!((Entry<K,V>)o instanceof Entry))//vemos eso si o si  ??
            {
                return false;
            }
            Entry<K, V> entry = (Entry<K, V>) o;

            int index = TSBHashTableDA.this.h(entry.getKey());
        ...
        private class EntrySetIterator implements Iterator<Map.Entry<K, V>>
        {
            ...
            public void remove()
            {
                ...
                TSBHashTableDA.this.remove(current_entry);
                next_ok = false;
                TSBHashTableDA.this.count--;
                TSBHashTableDA.this.modCount++;
                expected_modCount++;
